...
public class BoardMgr {
	...
	private static final String SAVEFOLER = "D:\\AJR_20230126\\wkh\\silsp\\p08_JSP\\Community_email_230426\\src\\main\\WebContent\\fileupload";
	...
	public int updateBoard(NBoardBean bean) {
		String sql = null;
		int exeCnt = 0;

		try {
			sql = "update noticeTblBoard set uname=?, subject=?, content=? where num=?";
...
/*  게시판 입력(/bbs/postProc.jsp) 시작  */
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			sql = "select max(num) from noticeTblBoard";
			pStmt = conn.prepareStatement(sql);
			...
			sql = "insert into noticeTblBoard (";
	...
	public Vector<NBoardBean> getBoardList(String keyField, String keyWord, int start, int end) {

		Vector<NBoardBean> vList = new Vector<>();
		String sql = null;
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				// 검색어가 없을 경우
				sql = "select * from noticeTblBoard "
			...
			while (rS.next()) {
				NBoardBean bean = new NBoardBean();
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			if(keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(*) from noticeTblBoard";
				pStmt = conn.prepareStatement(sql);
			} else {
				sql = "select count(*) from noticeTblBoard ";
	...
	public void upCount(int num) {
		...
		try {
			sql = "update noticeTblBoard set readcnt = readcnt+1 where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			pStmt.executeUpdate();
	...
	public NBoardBean getBoard(int num) {
		String sql = null;

		NBoardBean bean = new NBoardBean();
		try {
			sql = "select * from noticeTblBoard where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			rS = pStmt.executeQuery();
	...
	*/
	...
	public int deleteBoard(int num) {
		...
		try {
			...
			//////////// 게시글의 파일 삭제 시작 ///////////////
			sql = "select filename from noticeTblBoard where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			rS = pStmt.executeQuery();
			...
			//////////// 게시글 삭제 시작 ///////////////
			sql = "delete from noticeTblBoard where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			exeCnt = pStmt.executeUpdate();
			//////////// 게시글 삭제 끝 ///////////////
	...
	public int replyBoard(NBoardBean bean) {
	...
	public int replyUpBoard(int ref, int pos) {
		...
		try {
			...
			//////////// 게시글의 포지션 증가 시작 ///////////////
			sql = "update noticeTblBoard set pos = pos + 1 ";
