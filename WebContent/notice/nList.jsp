...
<body>
    <div id="wrap">
    	...
    	<main id="main" class="dFlex">
			...
			<%
			vList = nMgr.getBoardList(keyField, keyWord, start, end);  // DB에서 데이터 불러오기
			listSize = vList.size();			
  ...
  boolean hasAttachment = filename != null && !filename.isEmpty(); // 파일 첨부 여부
				...
				%>
					...
					<tr id="listBtnArea">
						<td colspan="2">
						<% if (uidSession == null) { %>
						<% } else if (uidSession.equals("admin")) { %> <!-- admin 계정인 경우에만 버튼 표시 -->
...
function movePage(p1) {
    ...
    let param = "/notice/nList.jsp" + queryString;
    location.href = param;
}
</script>
